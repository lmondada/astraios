...
import { useDefaultWorkerId } from "../workers/state";
...
const { reducer, createActions } = createReducer(initialNotebookState, {
  createNewCell: (
    ...
    const { cellId, before, defaultWorker: defaultWorkerId } = action;
    ...
    return {
      ...
      cellData: {
        ...state.cellData,
        [newCellId]: createCell({ id: newCellId, workerId: defaultWorkerId }),
      },
      cellRuntime: {
        ...state.cellRuntime,
  ...
  undoDeleteCell: (state) => {
    if (state.history.length === 0) {
      ...
      return {
        ...
        cellRuntime: {
          ...state.cellRuntime,
          // TODO(luca): restore workerId for undo
  ...
  updateCellWorker: (state, action: { cellId: CellId; workerId: string }) => {
    const { cellId, workerId } = action;
    return updateCellData(state, cellId, (cell) => {
      return {
        ...cell,
        workerId,
...
export function useSetDefaultWorkerIfEmpty() {
  ...
  Objects.keys(cellData).forEach((cellId) => {
    if (!cellData[cellId].workerId) {
      updateCellWorker({ cellId, workerId: defaultWorker });
    }
  });
}

...
import { useConnectedWorkers, useDefaultWorkerId } from "@/core/workers/state";
...
export function useCellActionButtons({ cell }: Props) {
  ...
  const { cellId, config, editorView, name, hasOutput, status, workerId } =
    cell;
  ...
  const actions: ActionButton[][] = [
    [
      ...
      },
      {
        ...
        rightElement: workerId ? (
          <WorkerDropdown
            value={workerId}
            options={connectedWorkers}
            onChange={(newId) => {
              updateCellWorker({ cellId, workerId: newId });
            }}
          />
        ) : (
